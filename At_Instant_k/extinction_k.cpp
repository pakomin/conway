...
long long int exbegin=3;
long long int exend=4;
...
void cause_extinction(vector<bool> arr[MAX_SIZE])
{
    long long int i,j;
    for(i=1;i<ROW_NUMBER-1;i++)
    {
        for(j=1;j<COL_NUMBER-1;j++)
        {
            ...
            if(i<(ROW_NUMBER/2) && j<(COL_NUMBER/2))
            {
...
pair<bool,long long int> life_state(vector<bool> arr[MAX_SIZE], long long int gener=-1)
{

    int i, j, count;
    pair<char, double> twoValues;
    if(gener>=exbegin && gener<=exend)
    {
        cause_extinction(arr);
    }
...
long long int run_simulation(vector<bool> gen0[MAX_SIZE],long long int endoftime=100)
{
   ...
   // system("clear");
    ...
    do
    {
       // system("sleep 0.1");
       // system("clear");
        pair<bool,long long int> anspair=life_state(gen0,genCount);
...
void userinteract()
{
    ...
    printf("Enter Extinction begin Time: ");
    scanf("%lld",&exbegin);
    printf("Enter Extinction begin Time: ");
    scanf("%lld",&exend);
...
void printsidebyside(vector<bool> gen1[MAX_SIZE],vector<bool> gen2[MAX_SIZE], long long int genCount)
{
    ...
    printf("Grid: %lld*%lld     TimeBound:%lld      Extinction:%lld-%lld",ROW_NUMBER-2,COL_NUMBER-2,upperbound,exbegin,exend);
