...
struct colony{
    ...
    vector<bool> cancer[MAX_SIZE];
};

long long int canceripos;
long long int cancerjpos;
...
pair<bool,long long int> life_state(vector<bool> arr[MAX_SIZE], vector<bool> cancer[MAX_SIZE])
{
    ...
    for (i = 1; i < ROW_NUMBER - 1; i++)
    {
        for (j = 1; j < COL_NUMBER-1; j++)
        {
            ...
            {
                if((arr[i][j]) && (cancer[i-1][j-1] || cancer[i-1][j] || cancer[i-1][j+1]  || cancer[i][j-1] || cancer[i][j+1] || cancer[i+1][j-1] || cancer[i+1][j] || cancer[i+1][j+1]))
                {
                    cancer[i][j]=true;
                }
                else
...
long long int run_simulation(vector<bool> gen0[MAX_SIZE],vector<bool> gencancer[MAX_SIZE],long long int endoftime=100)
{
    int i, j;
    char run_again;
    string sanity_check;
   ...
   // system("clear");
    ...
    do
    {
       // system("sleep 0.1");
       // system("clear");
        pair<bool,long long int> anspair=life_state(gen0,gencancer);
...
long long int initialize(vector<bool> arr[MAX_SIZE],vector<bool> c[MAX_SIZE])
{
...
void userinteract()
{
    ...
    canceripos = (rand() % (ROW_NUMBER-3)) +1;
    cancerjpos = (rand() % (COL_NUMBER-3)) +1;
...
//    int tsize = GA_TARGET.size();
    ...
    for (i = 0; i < POPULATION_SIZE; i++)
    {
        ...
        col->fitness = 0;
        //citizen.str.erase();
        long long int gotalive=initialize(col->gen,col->cancer);
...
//    int tsize = target.size();
    ...
    for (i = 0; i < POPULATION_SIZE; i++)
    {
        ...
        *temp = *(population[i]);
        ...
        fitness=run_simulation(temp->gen,temp->cancer,upperbound);
...
void mate1(col_vector &population, col_vector &buffer)// this is where the fucking happens
{
    ...
    for (i = esize; i < POPULATION_SIZE; i++)
    {
     //   buffer[i]=(colony*)malloc(sizeof(colony));
       ...
       /* 
        //mutate2
        ipos = (rand() % (ROW_NUMBER-3)) +1;
        jpos = (rand() % (COL_NUMBER-3)) +1;
            //mutate method 2
        for(jpos=1;jpos<COL_NUMBER-1;jpos++)
            (*buffer[i]).gen[ipos][jpos] = !((*buffer[i]).gen[ipos][jpos]);
        ...
        jpos = (rand() % (COL_NUMBER-3)) +1;
            //mutate method 2
      /  for(ipos=1;ipos<ROW_NUMBER-1;ipos++)
            (*buffer[i]).gen[ipos][jpos] = !((*buffer[i]).gen[ipos][jpos]);
        */
...
void printsidebyside(vector<bool> gen1[MAX_SIZE],vector<bool> gen2[MAX_SIZE], vector<bool> cancer1[MAX_SIZE],vector<bool> cancer2[MAX_SIZE],long long int genCount)
{
